...
import Deck10CRUD from '../../components/master-data/deck10Crud' // Updated import to Deck10CRUD
...
export default function Deck10() { // Updated component name
  const rowsPerPage = 10
  const { org } = useApp()
  const { addSnackbar } = useSnackbar()
  const [open, setOpen] = useState(false)
  const [deck10, setDeck10] = useState(null) // Updated state variable
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/deck10', { // Updated API endpoint
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
      ...
      setItems(res?.data?.deck10 || []) // Updated data key
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/deck10/${deleteRow.id}`) // Updated API endpoint
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`Deck10 item - ${deleteRow.name} deleted Successfully`) // Updated message
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length ? (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='deck10 table'>
            <TableHead>
              <TableRow>
