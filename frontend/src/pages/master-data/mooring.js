...
import MooringCRUD from '../../components/master-data/mooringCrud' // Update import path
...
export default function Mooring() {
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/mooring', {
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/mooring/${deleteRow.id}`)
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`Mooring item - ${deleteRow.name} deleted Successfully`)
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length > 0 && (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='mooring table'>
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {items.map((row, index) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>
                    <Button onClick={() => editRecord(row)} startIcon={<Edit />} color='primary'>Edit</Button>
                    <Button onClick={() => setDeleteRow(row)} startIcon={<Delete />} color='error'>Delete</Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <Dialog open={Boolean(deleteRow?.id)} onClose={() => setDeleteRow(null)}>
        ...
        <DialogContent>
          <Typography>Are you sure you want to delete {deleteRow?.name} {org.item}</Typography>
        </DialogContent>
        <DialogActions>
