...
import Deck11CRUD from '../../components/master-data/deck11Crud' // Updated import
...
export default function Deck11() {
  const rowsPerPage = 10
  const { org } = useApp()
  const { addSnackbar } = useSnackbar()
  const [open, setOpen] = useState(false)
  const [deck11, setDeck11] = useState(null)
  ...
  const handleSearchChange = (event) => {
    ...
    setSearchData({
      ...searchData,
      [name]:
        name === 'code' ? value.toUpperCase() : type === 'checkbox' ? checked : value
    })
  }
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/deck11', {
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/deck11/${deleteRow.id}`)
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`Deck11 item - ${deleteRow.name} deleted Successfully`)
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
            ...
            <EntityAutocomplete
              ...
              AutocompleteProps={{
                onChange: (event, value) => {
                  setSearchData({ ...searchData, entity: value })
                }
              }}
            />
          </Grid>
      ...
      {items.length ? (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='deck11 table'>
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {items.map((row, index) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>{row.entity && `${row.entity.name} (${row.entity.code})`}</TableCell>
                  <TableCell>
                    <Button onClick={() => editRecord(row)} startIcon={<Edit />} color='primary'>
                      Edit
                    </Button>
                    <Button onClick={() => setDeleteRow(row)} startIcon={<Delete />} color='error'>
                      Delete
                    </Button>
                  </TableCell>
                </TableRow>
      ...
      <Deck11CRUD open={open} handleClose={handleClose} id={deck11?.id} onSave={() => searchRecords(true)} />
