...
import Deck9CRUD from '../../components/master-data/deck9Crud' // Updated import
...
export default function Deck9() {
  const rowsPerPage = 10
  const { org } = useApp()
  const { addSnackbar } = useSnackbar()
  const [open, setOpen] = useState(false)
  const [deck9, setDeck9] = useState(null)
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/deck9', {
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/deck9/${deleteRow.id}`)
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`Deck9 item - ${deleteRow.name} deleted Successfully`)
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length ? (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='deck9 table'>
            <TableHead>
              <TableRow>
