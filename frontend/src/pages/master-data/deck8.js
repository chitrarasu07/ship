...
import Deck8CRUD from '../../components/master-data/deck8Crud' // updated import
...
export default function Deck8() {  // updated component name
  const rowsPerPage = 10
  const { org } = useApp()
  const { addSnackbar } = useSnackbar()
  const [open, setOpen] = useState(false)
  const [deck8, setDeck8] = useState(null)  // updated state variable
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/deck8', {  // changed endpoint
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
      ...
      setItems(res?.data?.deck8 || [])  // updated data key
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/deck8/${deleteRow.id}`)  // changed endpoint
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`Deck8 item - ${deleteRow.name} deleted Successfully`)  // updated message
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length ? (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='deck8 table'>
            <TableHead>
              <TableRow>
