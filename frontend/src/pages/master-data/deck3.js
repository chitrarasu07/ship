...
import Deck3CRUD from '../../components/master-data/deck3Crud' // CRUD component for deck3
...
export default function Deck3() {
  ...
  const [deck3, setDeck3] = useState(null)
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/deck3', {
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/deck3/${deleteRow.id}`)
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`Deck3 item - ${deleteRow.name} deleted Successfully`)
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length > 0 && (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='deck3 table'>
            <TableHead>
              <TableRow>
