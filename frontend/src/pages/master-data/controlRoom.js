...
import ControlRoomCRUD from '@/components/master-data/contolRoomCrud'
...
export default function ControlRoom() {
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/controlroom', {
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/controlroom/${deleteRow.id}`)
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`ControlRoom item - ${deleteRow.name} deleted Successfully`)
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length > 0 && (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='controlroom table'>
            <TableHead>
              <TableRow>
