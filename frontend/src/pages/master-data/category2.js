...
import Category2CRUD from '../../components/master-data/category2Crud'
...
export default function Category2() {
  ...
  const searchRecord = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/category2', {
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
      ...
      setCategories2(res?.data?.categories2 || [])
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/category2/${deleteRow.id}`)
      setDeleteRow(null)
      searchRecord(true)
      addSnackbar(`${org.category2} - ${deleteRow.name} deleted Successfully`)
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
  ...
  return (
    <>
      ...
      {!dataLoading && !categories2.length && !searchError ? (
        <Typography
          variant='body2'
          color='error'
          align='center'
