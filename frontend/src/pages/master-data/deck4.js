...
import Deck4CRUD from '../../components/master-data/deck4Crud' // CRUD component for deck4
...
export default function Deck4() {
  const rowsPerPage = 10
  const { org } = useApp()
  const { addSnackbar } = useSnackbar()
  const [open, setOpen] = useState(false)
  const [deck4, setDeck4] = useState(null)
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/deck4', {
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/deck4/${deleteRow.id}`)
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`Deck4 item - ${deleteRow.name} deleted Successfully`)
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length ? (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='deck4 table'>
            <TableHead>
              <TableRow>
