...
import Deck7CRUD from '../../components/master-data/deck7Crud' // Updated import for deck7
...
export default function Deck7() {  // Updated component name
  const rowsPerPage = 10
  const { org } = useApp()
  const { addSnackbar } = useSnackbar()
  const [open, setOpen] = useState(false)
  const [deck7, setDeck7] = useState(null)  // Updated state variable
  ...
  const newRecord = () => {
    setDeck7(null)  // Updated setter
    setOpen(true)
  }

  const editRecord = (row) => {
    setDeck7(row)  // Updated setter
    setOpen(true)
  }
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/deck7', {  // Updated API endpoint
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
      ...
      setItems(res?.data?.deck7 || [])  // Updated data key
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/deck7/${deleteRow.id}`)  // Updated API endpoint
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`Deck7 item - ${deleteRow.name} deleted Successfully`)  // Updated message
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length ? (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='deck7 table'>
            <TableHead>
              <TableRow>
