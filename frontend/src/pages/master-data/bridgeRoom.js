...
import BridgeRoomCRUD from '@/components/master-data/bridgeRoomCrud'
...
export default function BridgeRoom() {
  ...
  const [bridgeRoom, setBridgeRoom] = useState(null) // renamed state
  ...
  const searchRecords = async (getCount, pageNo) => {
    try {
      ...
      const res = await axios.get('/bridgeroom', { // Updated API endpoint
        params: {
          ...searchData,
          getCount,
          pageNo: pageNo + 1,
      ...
      setItems(res?.data?.bridgeroom || []) // Updated key
  ...
  const handleDelete = async () => {
    try {
      setSearchError('')
      setDataLoading(true)
      await axios.delete(`/bridgeroom/${deleteRow.id}`) // Updated API endpoint
      setDeleteRow(null)
      searchRecords(true)
      addSnackbar(`BridgeRoom item - ${deleteRow.name} deleted Successfully`) // Updated message
    } catch (error) {
      setDataLoading(false)
      setSearchError(error.response?.data?.error || error.response?.statusText)
      console.error(error)
    ...
    <>
      ...
      {items.length > 0 && (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='bridgeroom table'>
            <TableHead>
              <TableRow>
