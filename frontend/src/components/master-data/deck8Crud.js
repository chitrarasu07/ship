...
export default function Deck8CRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/deck8', { params: { id } }) // changed API endpoint
          const savedData = data?.deck8 && data.deck8[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Deck8 record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/deck8/${id}`, temp) // changed API endpoint
      } else {
        await axios.post('/deck8', temp) // changed API endpoint
      }
      setDataSaving(false)
      addSnackbar(`Deck8 ${formData.id ? 'updated' : 'created'} successfully`) // changed message
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Deck8') // changed error message
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit Deck8' : 'New Deck8'}</DialogTitle> {/* changed title */}
