...
export default function OutdoorCRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/outdoor', { params: { id } })
          const savedData = data?.outdoor && data.outdoor[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Outdoor record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/outdoor/${id}`, temp)
      } else {
        await axios.post('/outdoor', temp)
      }
      setDataSaving(false)
      addSnackbar(`Outdoor ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Outdoor')
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit Outdoor' : 'New Outdoor'}</DialogTitle>
