...
export default function Deck10CRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/deck10', { params: { id } })
          const savedData = data?.deck10 && data.deck10[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Deck10 record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/deck10/${id}`, temp)
      } else {
        await axios.post('/deck10', temp)
      }
      setDataSaving(false)
      addSnackbar(`Deck10 ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Deck10')
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit Deck10' : 'New Deck10'}</DialogTitle>
