...
export default function Deck1CRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/deck1', { params: { id } })
          const savedData = data?.deck1 && data.deck1[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Deck1 record not found')
          }
          console.log(data)
        } catch (error) {
          console.error(error)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/deck1/${id}`, temp)
      } else {
        await axios.post('/deck1', temp)
      }
      setDataSaving(false)
      addSnackbar(`Deck1 ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Deck1')
      console.error(err)
    }
  }
