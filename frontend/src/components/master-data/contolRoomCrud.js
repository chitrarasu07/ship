...
export default function ControlRoomCRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/controlroom', { params: { id } })
          const savedData = data?.controlroom && data.controlroom[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('ControlRoom record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/controlroom/${id}`, temp)
      } else {
        await axios.post('/controlroom', temp)
      }
      setDataSaving(false)
      addSnackbar(`ControlRoom ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save ControlRoom')
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit ControlRoom' : 'New ControlRoom'}</DialogTitle>
