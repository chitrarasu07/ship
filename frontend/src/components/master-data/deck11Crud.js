...
export default function Deck11CRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/deck11', { params: { id } })
          const savedData = data?.deck11 && data.deck11[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Deck11 record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/deck11/${id}`, temp)
      } else {
        await axios.post('/deck11', temp)
      }
      setDataSaving(false)
      addSnackbar(`Deck11 ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Deck11')
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit Deck11' : 'New Deck11'}</DialogTitle>
