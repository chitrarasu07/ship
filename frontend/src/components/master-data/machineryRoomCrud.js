...
export default function MachineryRoomCRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/machineryroom', { params: { id } })
          const savedData = data?.machineryroom && data.machineryroom[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('MachineryRoom record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/machineryroom/${id}`, temp)
      } else {
        await axios.post('/machineryroom', temp)
      }
      setDataSaving(false)
      addSnackbar(`MachineryRoom ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save MachineryRoom')
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit MachineryRoom' : 'New MachineryRoom'}</DialogTitle>
      <DialogContent dividers>
        ...
        >
          ...
          <EntityAutocomplete
            ...
            InputProps={{ error: errorField.entity }}
            AutocompleteProps={{
              required: true,
