...
export default function Deck6CRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/deck6', { params: { id } })
          const savedData = data?.deck6 && data.deck6[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Deck6 record not found')
          }
          console.log(data)
        } catch (error) {
          console.error(error)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/deck6/${id}`, temp)
      } else {
        await axios.post('/deck6', temp)
      }
      setDataSaving(false)
      addSnackbar(`Deck6 ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Deck6')
      console.error(err)
    }
  }
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit Deck6' : 'New Deck6'}</DialogTitle>
