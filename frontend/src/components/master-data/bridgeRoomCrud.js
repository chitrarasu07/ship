...
export default function BridgeRoomCRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/bridgeroom', { params: { id } })
          const savedData = data?.bridgeroom && data.bridgeroom[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('BridgeRoom record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/bridgeroom/${id}`, temp)
      } else {
        await axios.post('/bridgeroom', temp)
      }
      setDataSaving(false)
      addSnackbar(`BridgeRoom ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save BridgeRoom')
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle>{id ? 'Edit BridgeRoom' : 'New BridgeRoom'}</DialogTitle>
