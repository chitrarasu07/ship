...
export default function MooringCRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/mooring', { params: { id } })
          const savedData = data?.mooring && data.mooring[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Mooring record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/mooring/${id}`, temp)
      } else {
        await axios.post('/mooring', temp)
      }
      setDataSaving(false)
      addSnackbar(`Mooring ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Mooring')
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit Mooring' : 'New Mooring'}</DialogTitle>
