...
export default function Deck4CRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/deck4', { params: { id } })
          const savedData = data?.deck4 && data.deck4[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Deck4 record not found')
          }
          console.log(data)
        } catch (error) {
          console.error(error)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/deck4/${id}`, temp)
      } else {
        await axios.post('/deck4', temp)
      }
      setDataSaving(false)
      addSnackbar(`Deck4 ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Deck4')
      console.error(err)
    }
  }
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit Deck4' : 'New Deck14'}</DialogTitle>
