...
export default function Deck7CRUD(props) {
  ...
  useEffect(() => {
    ...
    if (id) {
      const fetchRecordById = async () => {
        try {
          setDataSaving(true)
          const { data } = await axios.get('/deck7', { params: { id } }) // <--- Changed endpoint
          const savedData = data?.deck7 && data.deck7[0]
          if (savedData) {
            setFormData({ ...formData, ...savedData })
          } else {
            setError('Deck7 record not found')
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(`/deck7/${id}`, temp) // <--- Changed endpoint
      } else {
        await axios.post('/deck7', temp) // <--- Changed endpoint
      }
      setDataSaving(false)
      addSnackbar(`Deck7 ${formData.id ? 'updated' : 'created'} successfully`)
      handleClose()
      onSave()
    } catch (err) {
      setDataSaving(false)
      setError(err.response?.data?.error || 'Failed to save Deck7')
    }
  }

  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{id ? 'Edit Deck7' : 'New Deck7'}</DialogTitle> {/* Changed title */}
